...
import models.{ EmployeeRepository, EmployeeRepositoryImpl, Employee }
 ...
 */
class EmployeeController(employeeRepository: EmployeeRepository) extends Controller {
  ...
  def list(page: Int, orderBy: Int, filter: String): Action[AnyContent] = Action.async { implicit request =>
    employeeRepository.list(page, 10, orderBy, "%" + filter + "%").map { pageEmp =>
  ...
  def edit(id: Long): Action[AnyContent] = Action.async { implicit request =>
    employeeRepository.findById(id).map(employee => Ok(html.editForm(id, employeeForm.fill(employee)))).recover {
      case ex: TimeoutException =>
  ...
  def update(id: Long): Action[AnyContent] = Action.async { implicit request =>
    employeeForm.bindFromRequest.fold(
      ...
      employee => {
        val futureEmpUpdate = employeeRepository.update(id, employee.copy(id = Some(id)))
  ...
  def save: Action[AnyContent] = Action.async { implicit request =>
    employeeForm.bindFromRequest.fold(
      ...
      employee => {
        val futureEmpInsert = employeeRepository.insert(employee)
...
object EmployeeController extends EmployeeController(EmployeeRepositoryImpl)
